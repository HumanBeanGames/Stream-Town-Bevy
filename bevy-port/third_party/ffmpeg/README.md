...
The built-in FFmpeg AAC encoder, Windows Media Foundation H.264 encoder, and
BSD-licensed OpenH264 fallback are sufficient for Twitch's H.264/AAC ingest.
...
The Windows packager includes the exact FFmpeg/OpenH264 source archives used by
vcpkg, the vcpkg port recipes and patches, installed SPDX manifests, and license
texts in `StreamTown/third_party/source`. The source archive hashes pinned by the
vcpkg ports are:
...
BSD-2-Clause. See the packaged copyright files and SPDX manifests for all
