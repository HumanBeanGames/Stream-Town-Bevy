...
- **RuntimeData is data-only** - RuntimeData classes must not store processors, command handlers, service objects, scene objects, or any other composed behavior references
...
- **Initialize never allocates RuntimeData** - `Initialize()` may only populate/validate installed runtime data, never first create it
...
- The owning processor may compose behavior/services separately, but RuntimeData remains a plain state container
