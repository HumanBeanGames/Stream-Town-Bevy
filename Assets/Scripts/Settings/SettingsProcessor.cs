...
//
// TODO(Architecture): Excluded from strict processor-template field conformance because this class is a UI/menu processor
// with transient UI state fields. This MAY need to be migrated to a non-processor pattern.
...
namespace Settings
{
    public class SettingsProcessor : MonoBehaviour, IInstaller, IProcessor
    {
        ...
        [Inject] GraphicsProcessor _graphics;
        ...
        private void ApplySettingsToEngine()
        {
            ...
            // Resolution (read from GraphicsProcessor list using stored index)
		...
		private void ApplyAutosaveIntervalFromCurrentSettings()
		{
			...
			if (CurrentSettings.autosaveTime < 0 || CurrentSettings.autosaveTime >= _autoSave.Intervals.Count)
				return;

			float autosaveSeconds = _autoSave.Intervals[CurrentSettings.autosaveTime] * 60.0f;
			_saveProcessor.AutosaveTime = autosaveSeconds;
			_saveProcessor.Autosave = autosaveSeconds > 0.0f;
		}
