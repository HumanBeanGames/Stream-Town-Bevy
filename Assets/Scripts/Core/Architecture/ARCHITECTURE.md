...
**Examples:**
- `ResourceSceneDataScriptable` - Stores resource lists (wood, ore, food, gold, recruits)
- `FoliageSceneDataScriptable` - Stores foliage lists (on land, underwater)
- `BuildingSettingsScriptable` - Stores building configuration (costs, max levels, ages)
...
**Verification:** Every IDataScriptable must have a corresponding SettingsInstaller in the scene's SceneScope.
...
- [ ] **Installer classes exempt** - Installer classes (SettingsInstaller, RuntimeInstaller) in `Core/Installers` are exempt as DI infrastructure; they may reference ScriptableObjects and use `ScriptablesProcessorInfrastructure` namespace
